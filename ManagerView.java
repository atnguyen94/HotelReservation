...
public class ManagerView {
   ...
   private void updateView() {
      ...
      String roomInfo = "Viewing room information for the date ";
      roomInfo += sdf.format(current.getTime()) + "\n\n";
      ArrayList<Reservation> reservationArrayList = model.getDateReservations(current.getTime());
      roomInfo += "Reserved Rooms:\n";
      ...
      for(Guest g : guests)
      {
         ...
         {
            ReservationLists lists = r.getReservations();
            Iterator<Reservation> reservationIterator = lists.getReservations();

            while(reservationIterator.hasNext())
            {
               Reservation res = reservationIterator.next();

               roomInfo += g.getName() + " reserved "
                       + "Room #" + r.getRoomNumber()
                     + ", their user id is " + g.getUserID()
                     + ", and the cost of the room is $" + res.getPrice() + "."
                     + '\n';
            }
         }
      }
      ...
      {
         if(!r.getReservations().getDateReservations(current.getTime()).hasNext()) {
            roomInfo += "Room #" + r.getRoomNumber() + '\n';
         }
      }
      textBox.setText(roomInfo);
   }
}
